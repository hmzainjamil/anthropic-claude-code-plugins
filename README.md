...
> **Anthropic Claude Code plugins — extend Claude with official plugin system**
...
| Concept | Description |
|---|---|
| **Plugins** | Core primitive of anthropic claude code plugins — fundamental building block |
| **Claude** | Execution layer handling claude operations and state |
| **Anthropic** | Integration point connecting anthropic to external services |
| **Extensions** | Configuration schema for extensions behavior and routing |
| **Mcp** | Optimization layer reducing MCP overhead and latency |
...
/skills load anthropic-claude-code-plugins

# Check skill status
/skills list | grep anthropic

# Clone and explore locally
gh repo clone hmzainjamil/anthropic-claude-code-plugins
cd anthropic-claude-code-plugins

# Run with Claude Code (invoke skill directly)
claude -p "use anthropic claude code plugins skill to handle my task"
...
- **Digital agencies** — automate plugins workflows, cut delivery time 10×
- **SaaS founders** — plug into existing Claude Code setup, zero infra overhead
- **Freelancers** — use as force-multiplier for client work in claude + anthropic
...
cp -r . ~/.claude/skills/anthropic-claude-code-plugins/

# Option 3: Via gh CLI
gh repo clone hmzainjamil/anthropic-claude-code-plugins ~/.claude/skills/anthropic-claude-code-plugins
```
...
response = client.messages.create(
    ...
    max_tokens=4096,
    system="You are using the anthropic-claude-code-plugins skill. Anthropic Claude Code plugins — extend Claude with official plugin system",
...
mae run "anthropic claude code plugins - execute primary workflow"

# Blast across multiple models
tcc blast "task1 for plugins" "task2 for claude" "task3 for anthropic"
```
...
```yaml
# .claude/skills/anthropic-claude-code-plugins/config.yaml
skill:
...
User Intent
    │
    ▼
Skill Router ──► Trigger Match ──► anthropic-claude-code-plugins
                                       │
                    ┌──────────────────┼──────────────────┐
                    ▼                  ▼                   ▼
...
```json
{
  "nodes": [
    {
      "name": "anthropic-claude-code-plugins Trigger",
    ...
    },
    {
      ...
      "parameters": {
        ...
        "body": {{"skill": "anthropic-claude-code-plugins", "input": "{{$json.body}}"}}
      }
    }
  ]
}
...
def run_skill(task: str) -> str:
    result = subprocess.run(
        ['claude', '-p', f'using anthropic-claude-code-plugins skill: {task}'],
        capture_output=True, text=True
    )
    return result.stdout

output = run_skill("plugins analysis for my project")
print(output)
```
...
1. Fork this repo
2. Create feature branch: `git checkout -b feature/plugins-improvement`
3. Add/update `SKILL.md` with new capabilities
4. Test: `claude -p "test anthropic-claude-code-plugins skill"`
...
### v1.0.0
- Initial release with core plugins functionality
