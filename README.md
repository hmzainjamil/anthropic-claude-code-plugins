...
> Plugin ecosystem for Anthropic Claude Code
...
|---------|-------------|--------|
| Core — Plugin foundation for plugins workflows | Core foundation — the primary abstraction this repo builds on | [docs](#) |
| Execution — task decomposition and plugins routing | Execution layer — how tasks get decomposed and routed | [docs](#) |
| Integration — Claude Code + claude-code connectivity | Integration layer — connecting to external tools and APIs | [docs](#) |
...
```bash
# Quick start
python3 main.py --task "plugins task here"

# Power user shortcut
python3 main.py --plugins --fast
...
Use anthropic-claude-code-plugins to automate plugin ecosystem for anthropic claude code. Perfect for agencies, freelancers, and AI-first teams running plugins workflows at scale.
...
```bash
# Add to CLAUDE.md
echo "Auto-activate: plugins, claude-code" >> ~/.claude/CLAUDE.md

# Or load skill directly
/load-skill anthropic-claude-code-plugins
```
...
{
  "hooks": {
    "PostToolUse": [{"matcher": "plugins", "command": "python3 ~/repos/anthropic-claude-code-plugins/main.py"}]
  }
}
```
...
```bash
# Fork and clone
gh repo fork hmzainjamil/anthropic-claude-code-plugins --clone
cd anthropic-claude-code-plugins
